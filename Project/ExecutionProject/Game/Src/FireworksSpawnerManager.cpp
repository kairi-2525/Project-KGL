	...
	if (ImGui::BeginMenuBar())
	{
		if (ImGui::BeginMenu(u8"ファイル"))
		{
			if (ImGui::BeginMenu(u8"新規作成"))
			{
				...
				{
					Create(set_name);
					debug_msg_mgr->AddMessage(TAG + " " + set_name + " を作成しました。", DebugMsgMgr::CL_SUCCESS, TAG);
					set_name.clear();
				}
				ImGui::EndMenu();
			}
			if (select_fs)
			{
				if (ImGui::BeginMenu(u8"書き出し"))
				{
					...
					{
						Export();
						debug_msg_mgr->AddMessage(TAG + " " + select_fs->name + " を書きしました。", DebugMsgMgr::CL_SUCCESS, TAG);
					}
					ImGui::EndMenu();
				}
			}
